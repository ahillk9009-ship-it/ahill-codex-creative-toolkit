...
const connectionSettings = require("./connection-settings.cjs").createConnectionSettings(storage.secureStorage);
let lastState = "";
let stateRevision = 0;
...
entrypoints.setup({
  panels: {
    mcpBridgePanel: {
      ...
      show() {
        if (connectionInitialized && (!socket || socket.readyState !== WebSocket.OPEN) && !reconnectTimer) {
          connectionStopped = false;
          connect();
        }
        publishState("panel.show");
      },
...
document.addEventListener("DOMContentLoaded", async () => {
  document.getElementById("connect").addEventListener("click", async () => {
    connectionStopped = false;
    try {
      await connectionSettings.save({ url: document.getElementById("bridge-url").value, token: document.getElementById("bridge-token").value });
    } catch (_) { setStatus("Connection settings could not be saved; reconnect manually after restart"); }
    connect();
  });
  ...
  subscribeHostEvents();
  const savedConnection = await connectionSettings.load();
  if (savedConnection) {
    document.getElementById("bridge-url").value = savedConnection.url;
    document.getElementById("bridge-token").value = savedConnection.token;
  }
...
function connect() {
  ...
  if (socket) try { socket.onopen = socket.onmessage = socket.onerror = socket.onclose = null; socket.close(); } catch (_) {}
  const configuredUrl = document.getElementById("bridge-url").value;
  const token = document.getElementById("bridge-token").value;
  let url;
  if (!token || token.length < 16) { setStatus("Enter the local bridge token, then Connect"); return; }
  try {
    url = WorkspaceSupport.validateLoopbackBridgeUrl(configuredUrl);
    if (token) url.searchParams.set("token", token);
  } catch (_) {
    setStatus("Invalid loopback bridge URL"); return;
  }
  setStatus("Connecting to " + url.origin + url.pathname);
  let current;
  try { current = socket = new WebSocket(url.toString()); } catch (_) { return scheduleReconnect("Bridge connection failed"); }
  const isCurrent = () => !connectionStopped && generation === connectionGeneration && current === socket;
  current.onopen = async () => {
    try {
      const hello = await capabilities();
      if (!isCurrent()) return;
      clearTimeout(connectTimer);
      current.send(Protocol.serializeEnvelope(Protocol.envelope("hello", hello)));
      setStatus("Connected — automatic reconnect enabled");
      publishState("connected");
    } catch (_) { if (isCurrent()) { current.close(); scheduleReconnect("Host handshake failed"); } }
  };
  current.onmessage = event => { if (isCurrent()) dispatch(event.data); };
  current.onerror = () => {
    if (!isCurrent()) return;
    try { current.close(); } catch (_) {}
    scheduleReconnect("Bridge connection error");
  };
  current.onclose = () => { if (isCurrent()) scheduleReconnect("Disconnected"); };
